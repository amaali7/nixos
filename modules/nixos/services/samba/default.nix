...
let
  ...
  inherit (lib) types mkEnableOption mkIf mapAttrs optionalAttrs;

  inherit (lib.amaali7) mkOpt mkBoolOpt;
  ...
  shares-submodule = with types;
    submodule ({ name, ... }: {
      options = {
        ...
        only-owner-editable = mkBoolOpt false
          "Whether the share is only writable by the system owner (amaali7.user.name).";

        extra-config =
          mkOpt attrs { } "Extra configuration options for the share.";
      };
    });
in
{
